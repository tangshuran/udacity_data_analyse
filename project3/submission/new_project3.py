...
import gc
file_path=r'D:\github\udacity_data_analyse\project3\report/sample3.osm'
...
def audit_postcode(file_path):
    """
    Adds potentially problematic postcode to
    dict 'problematic_postcode'
    """
        ...
        if elem.tag == "node" or elem.tag == "way":
            for tag in elem.iter("tag"):
                if tag.attrib['k'] == "addr:postcode":
                    test=re.match(r'^\d{5}$', tag.attrib['v'])
                    if not test:
...
def count_tags(filename):
    """
    returns the number of each tag type
    """
    data = {}
...
def update_name(name, mapping):
    """
    update the problamatic street name with predefined mapping
    """
...
def mapping_name(elem):
    """
    Return the element after updating street name
    """
    for tag in elem.iter("tag"):
        if is_street_name(tag):
...
def update_postcode(elem):
    """
    update the problamatic postcode
    """
    for tag in elem.iter("tag"):
        if tag.attrib['k'] == "addr:postcode":
            try:
...
def shape_element(element):
    """
    Use defined functions to shape the element into a whole dict object
    """
    obj={}
    created={}
    address={}
...
def process_map(file_in, pretty = False):
    """
    dump the shaped elements into the json file, prepared for the mongoimport
    """
...
def find_some_element(file_path):
    """
    return a element from the osm file, for testing
    """
